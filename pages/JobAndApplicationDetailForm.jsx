...
const JobAndApplicationDetailForm = ({ onSchedule }) => {

 // const [apiBaseUrl, setApiBaseUrl] = useState('http://localhost:8070');
  const [jobName, setJobName] = useState('');
  const [groupName, setGroupName] = useState('grc');
 ...
 const [jobRequestBody, setJobRequestBody] = useState('');
  ...
  const formatWithSeconds = (dateTime) => {
    ...
    useEffect(() => {
    axios.get(`${apiUrl}/jobschedular/apiList`,
      {
          headers: {
...
useEffect(() => {
  const fetchApplications = async () => {
    try {
      const response = await axios.get(`http://localhost/registerscimapp/registerfortenant/DEVINST/getAllApplications`);
  ...
  const handleSubmit = async (e) => {
    ...
    if (scheduleType === 'Once') {
      ...
      if (!startAt || !endAt || !frequencyInMinutes) {
        alert('Start time, end time and frequency are required for Recurring schedule.');
        return;
      }
      jobData = {
        ...jobData,
...
jobData.applicationData.jobRequestBody = parsedJson;
    ...
    try {
      ...
      // const pass = 'crave@123';
      const token = Buffer.from(`${user}:${pass}`).toString('base64');
      const response = await axios.post(`${apiUrl}/jobschedular/schedule`,
        jobData, {
          headers: {
            'Authorization': `Basic ${token}`
          }
    ...
    <form onSubmit={handleSubmit}>
      <div style={{ display: 'flex', gap: '20px' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          {(scheduleType === "Immediate" || scheduleType === "Once" || scheduleType === "Recurring") && (
            <label>
              Time Zone:
  ...
  <select
    ...
    onChange={(e) => {
      ...
      const selectedApp = applicationList.find(app => app.ApplicationName === selectedName);
      if (selectedApp) {
