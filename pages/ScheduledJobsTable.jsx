...
const paginatedData = filteredScheduledJobs.slice(
 ...
 //   if (!apiUrl) return;
    try {
      ...
      const response = await axios.get(`http://localhost/jobschedular/schJobList`, {
        headers: {
  ...
  useEffect(() => {
    console.log('Fetching jobs using URL:', apiUrl);
    fetchJobData();
  }, [apiUrl]);
        ...
        >
          <thead>
            <tr style={{ backgroundColor: "#f2f2f2" }}>
              ...
              <th className="table-td-date">Start At</th>
              <th className="table-td-date">End At</th>
          ...
          <tbody>
            {paginatedData.length === 0 ? (
              ...
              <td className="table-td-date">{job.startAt || '-'}</td>
              <td className="table-td-date">{job.endAt || '-'}</td>
